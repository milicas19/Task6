...
def get_distance_matrix(cells, distance_matrix_name, type="coordinate", neighborhood_radius=None, dist="Manhattan", param=None):
    ...
    if os.path.exists(f"matrices/{matrix_name}.npy"):
        ...
        distance_matrix = np.load(f"matrices/{matrix_name}.npy")
...
def plot_clusters(clusters, cells, node_colors, name, radius, text):
    ...
    for cell in cells:
        ...
        ax.scatter(x, y, color=color, s=0.2, alpha=1)
...
def cluster_leiden(graph, cells, name, radius, distance_matrix, num_of_types):
    ...
    homogeneity_score_1, homogeneity_score_2 = clusters_stat(partition.membership, num_of_types, cells, distance_matrix, name)
...
def cluster_agglomerative(distance_matrix, cells, name, radius, num_of_types, linkage_method, threshold=None):
    ...
    homogeneity_score_1, homogeneity_score_2 = clusters_stat(optimal_clusters, num_of_types, cells, distance_matrix, f"{name}_threshold_{int(optimal_threshold)}")

    text = f"silhouette_score: {max_score:.3f} number_of_clusters:{num_clusters} \nscore_1: {homogeneity_score_1:.4f}, score_2: {homogeneity_score_2:.4f}"
...
############ CLUSTER STATISTICS ############    
def cluster_homogeneity_score(cluster, cluster_mean, M):
    if cluster_mean > M:
        print(f"WARNING: Cluster {cluster} distance mean greater than mean of all distances")
        return 0
    return 1 - cluster_mean/M

def is_cluster_homogeneous(cluster, cell_ids, percentage_distance_matrix, M, name):
    start_time = datetime.now()
    ...
    if n == 1:
        ...
        cluster_mean = np.mean(distances)
        homogeneity_score = cluster_homogeneity_score(cluster, cluster_mean, M)
    
    print(f"cluster distances MEAN: {cluster_mean}")
    print(f"cluster homogeneity score: {homogeneity_score:.4f}")
    
    plot_values(distances, f"{name}_isHomogeneous_cluster_{cluster}", "Distances", "Number of cell pairs with specified distance", 
                f"cluster {cluster} distances -> MEAN: {cluster_mean}, score: {homogeneity_score:.4f}")
    print(f"###### job completed in: {datetime.now() - start_time}")
    
    return homogeneity_score
    
def cluster_stat(cluster, cell_ids, num_of_types, cells, percentage_distance_matrix, M, name):
    start_time = datetime.now()
    ...
    homogeneity_score = is_cluster_homogeneous(cluster, cell_ids, percentage_distance_matrix, M, f"{name}_isHomogeneous_cluster_{cluster}")
...
def clusters_stat(clusters, num_of_types, cells, percentage_distance_matrix, name):
    ...
    M = np.mean(upper_triangle_values)
    print(f"all distances MEAN: {M}")
    ...
    homogeneity_score_1 = 0
    homogeneity_score_2 = 0
    for cluster in clusters_dict:
        cell_ids = clusters_dict[cluster]
        cluster_homogeneity_score = cluster_stat(cluster, cell_ids, num_of_types, cells, percentage_distance_matrix, M, name)
        homogeneity_score_1 += cluster_homogeneity_score
        homogeneity_score_2 += cluster_homogeneity_score * len(cell_ids)
    
    homogeneity_score_1 /= len(clusters_dict)
    homogeneity_score_2 /= len(clusters)
    
    print(f"###### job completed in: {datetime.now() - start_time}")
    
    return homogeneity_score_1, homogeneity_score_2
